  ...
  return (
    <div className="w-[95%] transition-all duration-700 mx-auto mt-5">
      ...
      {MyEntroll?.payload.map((myCorurse: any) => (
        <Card className="w-[26%]  rounded-md border border-gray-300 pb-3 ">
          <img
          ...
          <p className="my-2 w-[90%] mx-auto">Lesson Complted {myCorurse?.courseId?.lessons.length}/{myCorurse?.progress?.completedLessons?.length}</p>
          <button onClick={()=> router.push(`/courses/${myCorurse?.courseId?._id}`)} className="px-5 py-2 bg-black text-white mt-3 ml-3 rounded-md">
            {" "}
           Continue
          </button>
        </Card>
